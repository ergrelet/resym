...
fn find_unnamed_unions_in_struct(fields: &[Field]) -> Vec<Range<usize>> {
    ...
    let mut unions_found_temp: BTreeMap<u32, (Range<usize>, u32)> = BTreeMap::new();

    // Discover unions
    let mut curr_union_offset_range: Range<u32> = Range::default();
    for (i, field) in fields.iter().enumerate() {
        ...
        if curr_union_offset_range.contains(&field.offset) {
            ...
            union_info.1 = std::cmp::max(union_info.1, field.size as u32);
            curr_union_offset_range.end = std::cmp::max(
                curr_union_offset_range.end,
                field.offset + field.size as u32,
            );
        } else {
            match unions_found_temp.get_mut(&field.offset) {
                Some(union_info) => {
                    ...
                    for previous_field in &fields[union_info.0.clone()] {
                        // Update the union's size
                        union_info.1 = std::cmp::max(union_info.1, previous_field.size as u32);
                        curr_union_offset_range.end = std::cmp::max(
                            curr_union_offset_range.end,
                            previous_field.offset + previous_field.size as u32,
                        );
                    }
                }
                None => {
                    unions_found_temp.insert(
                        field.offset,
                        (Range { start: i, end: i }, field.size as u32),
                    );
                }
            }
        }
...
fn find_unnamed_structs_in_unions(fields: &[Field]) -> Vec<Range<usize>> {
    ...
    let union_offset: u32 = fields[0].offset;
    let mut previous_field_offset: u32 = fields[0].offset;
    for (i, field) in fields.iter().enumerate() {
