    ...
    fn reconstruct(
        ...
        for value in &self.values {
            writeln!(
                ...
                match value.value {
                    pdb::Variant::U8(v) => { if _fmt_configuration.use_hexadecimal { format!("0x{v:02x}") } else { format!("{v}") } },
                    pdb::Variant::U16(v) => { if _fmt_configuration.use_hexadecimal { format!("0x{v:04x}") } else { format!("{v}") } },
                    pdb::Variant::U32(v) => { if _fmt_configuration.use_hexadecimal { format!("0x{v:08x}") } else { format!("{v}") } },
                    pdb::Variant::U64(v) => { if _fmt_configuration.use_hexadecimal { format!("0x{v:16x}") } else { format!("{v}") } },
